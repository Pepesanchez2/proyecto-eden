...
public partial class Player : CharacterBody2D
{
	...
	try { EmitSignal("HealthChanged", Health, MaxHealth); } catch { }
		...
		// Cargar arma inicial seleccionada en el menú
		LoadInitialWeapon();
	}
	...
	private void LoadInitialWeapon()
	{
		try
		{
			// Obtener el arma seleccionada desde el menú
			var buttonSelect = button_select.Instancia;
			if (buttonSelect != null && !string.IsNullOrEmpty(buttonSelect.armaSeleccionada))
			{
				string weaponId = buttonSelect.armaSeleccionada;

				// Instanciar la escena del arma
				var path = $"res://scenes/weapons/{weaponId.ToLower()}.tscn";
		...
		{
			GD.PrintErr($"Error cargando arma inicial: {ex.Message}");
		}
	}
